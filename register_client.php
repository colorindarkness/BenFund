...
<?php
if ($_POST['accept'] != 'set'){
	die("<h1>You must accept the User Agreement and Privacy Policy to register for an account</h1><p><a href='javascript: history.go(-1)'><img style='width: 125px; height: 50px;' src='https://www.benfund.com/images/elements/back.gif' alt='Back' border='0'></a>");
} else {
?>
		<?php if ($_POST['submit'] == Next){ ?>
		<?php require ($ROOT."/includes/register_client/validate.php"); ?>
			<?php if (isset($err)){ ?>
				<?php include ($ROOT."/includes/register_client/error.php"); ?>
				<?php } else { ?>
						<?php include ($ROOT."/includes/register_client/review.php"); ?>	
				<?php } ?>		
	<?php } else { ?>
	<?php include ($ROOT."/includes/register_client/default.php"); ?>
	<?php } ?>
<?php } ?>
	</div>
	</div>
